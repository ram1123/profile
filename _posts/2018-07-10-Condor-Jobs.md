...
1. Use the `condor_history` command to get a list of recently completed job IDs. You can use the following command: `condor_history <USER-NAME>`
    ...
    1. ***This command can be particularly helpful if you have a large number of submitted Condor jobs, and you need to identify a small subset of jobs that have recently completed.***
...
1. **Change of schedular at lxplus**: If you want to switch to schedular named `bigbird17` then use command:
   ```bash
   export _condor_SCHEDD_HOST=bigbird17.cern.ch
   export _condor_CREDD_HOST=bigbird17.cern.ch
   ```

1. Add the system requirement for SL9
